...
import { Plus, Trash2, Search, Receipt, Pencil } from "lucide-react";
...
export default function Transactions({ user, transactions, onAdd, onUpdate, onDelete, onAudit }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingId && onUpdate) {
      ...
      onAudit?.("Transaction Updated", `${form.type} - ${form.category} - KES ${form.amount} by ${user.name}`);
    } else {
      onAdd({
        type: form.type,
        category: form.category,
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button onClick={() => { setEditingId(null); setForm({ type: "income", category: "", amount: "", description: "", date: new Date().toISOString().split("T")[0], receipt: "" }); }}><Plus className="w-4 h-4 mr-2" />Add Transaction</Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader><DialogTitle className="font-serif">{editingId ? "Edit Transaction" : "New Transaction"}</DialogTitle></DialogHeader>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-muted-foreground border-b border-border bg-muted/30">
                ...
                {user.role === "admin" && <th className="px-5 py-3 font-medium">Recorded By</th>}
              ...
              <AnimatePresence>
                {filtered.slice(0, 50).map((t) => (
                  ...
                  >
                    ...
                    {user.role === "admin" && <td className="px-5 py-3 text-muted-foreground">{t.recordedBy}</td>}
                    ...
                    <td className="px-5 py-3">
                      {user.role === "admin" && (
                        <div className="flex items-center gap-2">
                          <button onClick={() => handleEditClick(t)} className="text-muted-foreground hover:text-primary transition-colors">
